...
//}
uint8_t *removeZeros(uint8_t *substruct, int *substractSize)
{
    int i;
    uint8_t *newSubstruct;
    for (i = 0; i < *substractSize; i++)
    {
        if (*(substruct + i) != 0)
        {
            newSubstruct = (substruct + i);
            *substractSize = *substractSize - i;
            break;
        }
    }
    return newSubstruct;
